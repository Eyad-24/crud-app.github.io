...
export default function TableList({handleOpen, tableData, setTableData, searchTerm}){
    ...
    const handleDelete = async (id) => {
        ...
        if (confirmDelete) {
            try {
                await axios.delete(`http://localhost:3007/api/clients/${id}`); // API call to delete client
    ...
    return(
        ...
        <tr key={client.id}>
            ...
            <td>
                <button className="btn btn btn-error" onClick={() => handleDelete(client.id)}>Delete</button>
            </td>
        </tr>
        ))} 
        </tbody>
