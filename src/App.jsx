...
function App(){
  ...
  const fetchClients = async () => {
      try{
          ...
          setTableData(response.data); // Set the fached data
      } catch (err) {
  ...
  const handleSubmit = async (newClientData) => {
    if (modalMode === 'add') {
      try {
        const response = await axios.post('http://localhost:3007/api/clients', newClientData); 
      ...
      console.log('Updating client with ID:', clientData.id); 
            try {
                const response = await axios.put(`http://localhost:3007/api/clients/${clientData.id}`, newClientData);
